    ...
    <section className="py-12 md:py-20 px-4 sm:px-6 bg-gradient-to-br from-gray-50 via-cyan-50 to-blue-50">
      <div className="container mx-auto max-w-6xl">
        ...
        >
          ...
          />
          <motion.p
            className="text-lg sm:text-xl text-gray-700 max-w-3xl mx-auto font-medium px-4"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.7 }}
          >
        ...
        <motion.div
          ...
          animate={inView ? "visible" : "hidden"}
          variants={containerVariants}
          className="grid lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16 mb-16 md:mb-20 items-start"
        >
          {/* Left Content */}
          <motion.div
            variants={itemVariants}
          ...
          >
            ...
            >
              <motion.div
                className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl md:rounded-2xl p-4 md:p-6 text-white shadow-xl relative overflow-hidden group cursor-pointer"
                variants={cardVariants}
                whileHover="hover"
              >
                {/* Animated Background */}
                ...
                <motion.h3
                  className="text-xl md:text-2xl font-bold mb-3 md:mb-4 flex items-center relative z-10"
                  variants={itemVariants}
                >
                  <motion.div
                ...
                <motion.p
                  className="text-blue-50 leading-relaxed relative z-10 text-sm md:text-base"
                  variants={itemVariants}
                >
                  To create a bridge between aspiring candidates and leading IT
                  companies by delivering high-quality, job-oriented training
                ...
                {/* Floating elements */}
                <motion.div
                  className="absolute top-3 md:top-4 right-3 md:right-4 w-2 h-2 md:w-3 md:h-3 bg-white rounded-full"
                  animate={pulseAnimation}
                />
              </motion.div>

              <motion.div
                className="bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl md:rounded-2xl p-4 md:p-6 text-white shadow-xl relative overflow-hidden group cursor-pointer"
                variants={cardVariants}
                whileHover="hover"
              >
                {/* Animated Background */}
                ...
                <motion.p
                  className="text-cyan-50 leading-relaxed relative z-10 text-sm md:text-base"
                  variants={itemVariants}
                >
                  To transform fresh graduates into job-ready professionals
                  through hands-on learning, mentorship, and corporate exposure,
                ...
                {/* Floating elements */}
                <motion.div
                  className="absolute bottom-3 md:bottom-4 right-3 md:right-4 w-1.5 h-1.5 md:w-2 md:h-2 bg-white rounded-full"
                  animate={{
                    ...pulseAnimation,
                    scale: [1, 1.5, 1],
                  }}
          ...
          <motion.div variants={itemVariants} className="relative">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {codingImages.map((image, index) => (
                <motion.div
                  key={index}
                  className={`relative rounded-lg md:rounded-2xl overflow-hidden shadow-lg md:shadow-xl group cursor-pointer ${
                    index === 0 || index === 3
                ...
                >
                  ...
                  {/* Image Icon */}
                  <motion.div
                    className={`absolute top-2 left-2 w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br ${image.color} rounded-lg md:rounded-xl flex items-center justify-center text-white shadow-lg`}
                    whileHover={{ scale: 1.2, rotate: 360 }}
                    transition={{ duration: 0.5 }}
                  >
                    <image.icon className="text-sm md:text-lg" />
                  </motion.div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-2 md:p-3 text-white transform translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                    ...
                    <p className="text-xs text-blue-100 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden sm:block">
                      {image.description}
                    </p>
                  </div>
                  ...
                  <motion.div
                    className="absolute top-1 right-1 bg-white/90 backdrop-blur-sm rounded p-1 md:p-2 shadow-lg hidden sm:block"
                    animate={floatingAnimation}
                  >
                    <div className="text-xs font-mono text-blue-600">
                      <div className="text-cyan-400 text-xs">fn</div>
                      <div className="text-blue-600 text-xs">code()</div>
                    </div>
                  </motion.div>
                </motion.div>
              ))}
            ...
            <motion.div
              className="absolute -top-4 -right-4 w-16 h-16 md:w-20 md:h-20 bg-blue-200 rounded-full -z-10 hidden md:block"
              animate={{
                scale: [1, 1.2, 1],
                opacity: [0.5, 0.8, 0.5],
              }}
            ...
            />
            <motion.div
              className="absolute -bottom-4 -left-4 w-12 h-12 md:w-16 md:h-16 bg-cyan-200 rounded-lg -z-10 hidden md:block"
              animate={{
                scale: [1, 1.1, 1],
                opacity: [0.3, 0.6, 0.3],
              }}
        ...
        >
          <motion.h2
            className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-900 mb-8 md:mb-12 px-4"
            initial={{ opacity: 0 }}
            whileInView={{ opacity: 1 }}
            transition={{ delay: 0.3 }}
          >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {stats.map((stat, index) => (
              <motion.div
                key={index}
                className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg md:shadow-xl border border-blue-100 text-center group cursor-pointer"
                variants={cardVariants}
                initial="hidden"
                whileInView="visible"
                whileHover="hover"
              ...
              >
                ...
                >
                  <stat.icon
                    className={`text-2xl md:text-4xl bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}
                  />
                </motion.div>

                <motion.div
                  className={`text-xl md:text-3xl font-bold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent mb-1 md:mb-2`}
                  animate={{ scale: [1, 1.1, 1] }}
                  transition={{
                    duration: 2,
                    repeat: Infinity,
                ...
                {/* Hover effect line */}
                <motion.div
                  className={`h-1 bg-gradient-to-r ${stat.color} rounded-full mt-3 md:mt-4 opacity-0 group-hover:opacity-100`}
                  initial={{ width: 0 }}
                  whileHover={{ width: "100%" }}
                  transition={{ duration: 0.3 }}
                />
        ...
        <motion.div
          ...
          transition={{ duration: 0.8 }}
          viewport={{ once: true }}
          className="bg-gradient-to-br from-blue-600 via-cyan-600 to-indigo-700 rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-12 text-white relative overflow-hidden"
        >
          {/* Background Pattern */}
          <div className="absolute inset-0 opacity-10">
            <motion.div
              className="absolute top-0 left-0 w-24 h-24 md:w-32 md:h-32 bg-white rounded-full -translate-x-1/2 -translate-y-1/2"
              animate={floatingAnimation}
            />
            <motion.div
              className="absolute bottom-0 right-0 w-16 h-16 md:w-24 md:h-24 bg-white rounded-full translate-x-1/2 translate-y-1/2"
              animate={{
                ...floatingAnimation,
                y: [0, -12, 0],
              }}
          ...
          {/* Animated Elements */}
          <motion.div
            className="absolute top-4 right-4 md:top-8 md:right-8 w-12 h-12 md:w-16 md:h-16 bg-white/20 rounded-full"
            animate={{
              scale: [1, 1.5, 1],
              opacity: [0.3, 0.6, 0.3],
            }}
          ...
          <motion.h2
            className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 md:mb-12 text-center relative z-10 px-4"
            initial={{ scale: 0.9 }}
            whileInView={{ scale: 1 }}
            transition={{ duration: 0.5 }}
          >
          ...
          <motion.div
            className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 relative z-10"
            variants={containerVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
          >
            {whyChooseUs.map((item, index) => (
              <motion.div
                key={index}
                className="bg-white/10 backdrop-blur-sm rounded-xl md:rounded-2xl p-4 md:p-6 group cursor-pointer border border-white/20"
                variants={cardVariants}
                whileHover="hover"
                custom={index}
              >
                <div className="flex items-start space-x-3 md:space-x-4">
                  <motion.div
                    className={`w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br ${item.color} rounded-lg md:rounded-xl flex items-center justify-center text-white shadow-lg flex-shrink-0`}
                    variants={iconVariants}
                    whileHover="hover"
                  >
                  ...
                  <motion.span
                    className="font-semibold text-base md:text-lg text-white/95 leading-relaxed"
                    whileHover={{ color: "rgba(255,255,255,1)" }}
                  >
                    {item.text}
                  </motion.span>
                ...
                {/* Hover border effect */}
                <motion.div
                  className={`h-1 bg-gradient-to-r ${item.color} rounded-full mt-3 md:mt-4 opacity-0 group-hover:opacity-100`}
                  initial={{ width: 0 }}
                  whileHover={{ width: "100%" }}
                  transition={{ duration: 0.3 }}
                />
          ...
          >
            <motion.button
              className="bg-white text-blue-700 font-bold py-3 px-8 md:py-4 md:px-12 rounded-full text-base md:text-lg shadow-xl md:shadow-2xl hover:shadow-3xl transition-all duration-300 border-2 border-white group w-full sm:w-auto"
              whileHover={{
                scale: 1.05,
                backgroundColor: "#f0f9ff",
                boxShadow: "0 25px 50px rgba(0,0,0,0.3)",
